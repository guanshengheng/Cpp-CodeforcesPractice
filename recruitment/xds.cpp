...
void solve()
{
    ...
    // 1. 建树
    build(1, n, 1);
    cout << "初始区间 [1, 5] 的和: " << que(1, 5, 1, n, 1) << " (期望值: 15)" << endl;

    // 2. 区间修改：将 [2, 4] 每个数加 10
    // 原数组变成: {1, 12, 13, 14, 5}
    mfy(2, 4, 10, 1, n, 1);
    cout << "修改后区间 [2, 4] 的和: " << que(2, 4, 1, n, 1) << " (期望值: 39)" << endl;
    cout << "修改后区间 [1, 5] 的和: " << que(1, 5, 1, n, 1) << " (期望值: 45)" << endl;

    // 3. 单点查询验证 (查询 [3, 3])
    cout << "修改后下标为 3 的值: " << que(3, 3, 1, n, 1) << " (期望值: 13)" << endl;
}
